...
class AssinaturaSupabaseService {
    ...
    if (this.supabase) {
      try {
        ...
        const supabaseData = {
          ...
          contract_title: (config as any).contract_title,
          clauses: (config as any).clauses
        };
  ...
  async deleteContract(id: string, tenantId?: string): Promise<boolean> {
    ...
    try {
      // 🔐 MULTI-TENANT: filtrar por tenant_id quando disponível
      let query = this.supabase.from('contracts').delete().eq('id', id);
      if (tenantId) {
  ...
  async deleteContractByToken(token: string, tenantId?: string): Promise<boolean> {
    // Garantir inicialização antes de usar
    await this.ensureInitialized();
    ...
    try {
      // 🔐 MULTI-TENANT: filtrar por tenant_id quando disponível
      let query = this.supabase.from('contracts').delete().eq('access_token', token);
      if (tenantId) {
        query = query.eq('tenant_id', tenantId);
      }
      const { error } = await query;
