...
export async function registerRoutes(app: Express) {
  ...
  app.get("/api/kanban-leads", requireTenant, async (req, res) => {
    try {
      // 🔐 MULTI-TENANT: filter by the authenticated tenant's ID
      const tenantId: string = (req as any).tenantId || (req as any).session?.tenantId;
      const leads = await kanbanStorage.getLeadsByTenant
        ? await kanbanStorage.getLeadsByTenant(tenantId)
        : (await kanbanStorage.getAllLeads()).filter((l: any) => !tenantId || l.tenant_id === tenantId || l.tenantId === tenantId);
      res.json(leads);
    } catch (error) {
