...
export function setupConfigRoutes(app: Express) {
  ...
  app.post("/api/config/forms/public-settings", authenticateToken, async (req: AuthRequest, res) => {
    try {
      ...
      if (markAllPublic) {
        ...
        // 🔐 MULTI-TENANT: Sempre filtrar por tenant_id para não afetar outros tenants
        let updateQuery = supabase.from('forms').update({ is_public: true }).eq('tenant_id', tenantId);
  ...
  app.post("/api/config/company-slug", authenticateToken, async (req: AuthRequest, res) => {
    try {
      ...
      const { companySlug, companyName } = req.body;
      if (!companySlug || typeof companySlug !== 'string') {
        return res.status(400).json({ error: 'companySlug é obrigatório' });
      }

      // 🔄 RENAME TENANT CASCADE PARA AS 157 TABELAS!
      if (companySlug && companySlug !== tenantId) {
        console.log(`[POST /api/config/company-slug] 🚀 Iniciando Migração de Tenant nas 157 Tabelas: '${tenantId}' -> '${companySlug}'`);
        try {
          ...
          // 🔑 Buscar credenciais Supabase do banco PostgreSQL (não de env vars)
          // Prioridade: env vars → banco (primeira entry com URL válida não-criptografada)
          let masterUrl = process.env.SUPABASE_LOCAL_URL || process.env.REACT_APP_SUPABASE_URL || '';
          let masterKey = process.env.SUPABASE_LOCAL_SERVICE_KEY || process.env.REACT_APP_SUPABASE_ANON_KEY || '';

          if (!masterUrl || !masterKey) {
            const { pool: pgPool } = await import('../db');
            const credRow = await pgPool.query(
            ...
            if (credRow.rows.length > 0) {
              masterUrl = credRow.rows[0].supabase_url;
              masterKey = credRow.rows[0].supabase_anon_key;
              console.log(`🔑 [Cascade] Credenciais Supabase obtidas do banco PostgreSQL: ${masterUrl.substring(0, 40)}`);
            }
          }

          if (!masterUrl || !masterKey) {
             throw new Error("Master credentials not found in env vars or supabase_config table!");
          }
          ...
          try {
             ...
             console.error("Erro carregando json de tabelas:", e);
             tablesList = ["forms", "tenants_registry", "form_submissions", "reunioes"];
          }
          
          console.log(`🔄 Atualizando ${tablesList.length} tabelas remotas (Supabase Endpoint via Master Key)...`);
          const updatePromises = tablesList.map(async (table) => {
              try {
                  if (table === 'tenants_registry') {
                     return await masterClient.from(table).update({ tenant_id: companySlug, tenant_slug: companySlug }).eq('tenant_id', tenantId);
                  }
                  return await masterClient.from(table).update({ tenant_id: companySlug }).eq('tenant_id', tenantId);
              } catch (err: any) {
          ...
          console.log("✅ Migração de 157 tabelas remota via REST concluída com sucesso!");

          // === CASCATA POSTGRESQL LOCAL ===
          // Busca dinamicamente todas as tabelas com coluna tenant_id
          ...
          try {
            const tablesResult = await pool.query(`
              ...
              ORDER BY table_name
            `);
            const pgTables = tablesResult.rows.map((r: any) => r.table_name);
            console.log(`🔄 Atualizando ${pgTables.length} tabelas PostgreSQL local...`);
            for (const table of pgTables) {
              try {
                ...
                console.warn(`[PG cascade] Tabela ${table}: ${pgErr.message}`);
              }
            }
            console.log(`✅ PostgreSQL cascade concluído para ${pgTables.length} tabelas`);
          } catch (pgCascadeErr: any) {
            console.warn('[PG cascade] Erro ao buscar tabelas:', pgCascadeErr.message);
          }

          // Invalidar publicCache do tenant antigo e novo
          const { invalidateTenantCache } = await import('../lib/publicCache');
          invalidateTenantCache(tenantId);
          invalidateTenantCache(companySlug);

          // Salvar companyName se fornecido
          if (companyName && typeof companyName === 'string') {
            try {
              const { appSettings: appSettingsTable } = await import('../../shared/db-schema.js');
              await db.update(appSettingsTable)
                .set({ companyName: companyName.trim(), updatedAt: new Date() })
                .where(eq(appSettingsTable.tenantId, companySlug));
            } catch (nameErr: any) {
              console.warn('[config] Erro ao salvar companyName:', nameErr.message);
            }
          }

          // BUG 7: Atualizar Sessão e JWT para o novo tenant_id soberano
          if (req.session) {
            req.session.tenantId = companySlug;
            req.session.userId = companySlug; // Se userId é usado como tenantId
          }

        } catch (renameErr) {
          console.error("❌ Erro grave ao tentar migração em lote nas 157 tabelas:", renameErr);
        }
      }

      // Invalidar slug cache do tenant antigo antes do saveCompanySlug
      const { saveCompanySlug, invalidateSlugCache } = await import('../lib/tenantSlug');
      invalidateSlugCache(tenantId);
      // Após cascade, app_settings.tenant_id já foi renomeado para companySlug
      // Então o lookup deve usar o novo slug como chave
      const lookupKey = companySlug !== tenantId ? companySlug : tenantId;
      const saved = await saveCompanySlug(lookupKey, companySlug);

      if (saved) {
        const { getCompanySlug } = await import('../lib/tenantSlug');
        const normalizedSlug = await getCompanySlug(companySlug);
        res.json({ success: true, companySlug: normalizedSlug, migrateTriggered: companySlug !== tenantId });
      } else {
