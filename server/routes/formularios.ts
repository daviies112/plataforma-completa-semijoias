...
router.put('/config/ativo', async (req, res) => {
  try {
    ...
    // Tenta primeiro via cliente do tenant; se falhar, usa fallback com SUPABASE_LOCAL_URL
    let supabaseForSync = await getSupabaseClientForFormularios(req);

    // 🔐 FALLBACK: Se client do tenant não disponível, usa env vars diretamente
    if (!supabaseForSync) {
      ...
      if (fallbackUrl && fallbackKey && fallbackUrl.startsWith('http')) {
        supabaseForSync = createClient(fallbackUrl, fallbackKey);
        console.log('⚠️ [FORMS] Usando Supabase LOCAL (env fallback) para sincronizar app_settings');
      }
    }
    ...
    if (supabaseForSync) {
      try {
        ...
        const { error: supabaseUpdateError } = await supabaseForSync
          .from('app_settings')
        ...
        if (supabaseUpdateError) {
          ...
          console.log(`✅ [FORMS] Supabase app_settings sincronizado: active_form_url=${formUrl}`);
        }
      } catch (e) {
        console.warn('⚠️ [FORMS] Não foi possível sincronizar com Supabase:', e);
      }
    } else {
      console.warn('⚠️ [FORMS] Supabase indisponível - sincronização ignorada (apenas Postgres local atualizado)');
    }
