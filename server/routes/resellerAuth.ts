...
if (!_rawSecret) {
  console.error('[resellerAuth] ❌ CRÍTICO: JWT_SECRET não configurado! Tokens não persistirão entre restarts.');
}
// Secret randômico por inicialização se não configurado (não prod-safe)
const JWT_SECRET = _rawSecret || require('crypto').randomBytes(64).toString('hex');
const JWT_EXPIRY = '7d';
...
router.post('/login', async (req: Request, res: Response) => {
  try {
    ...
    // Pró credenciais de teste (apenas se NODE_ENV=test explicitamente)
    const isTestEnv = process.env.NODE_ENV === 'test';
    const isTestCredentials = email === 'teste@upvendas.com' && cpfNormalizado === '12345678900';

    if (isTestEnv && isTestCredentials) {
      console.log('[NEXUS-TEST] Login de teste para revendedora mock');
      // Não permitido em production ou development — só em NODE_ENV=test
      req.session.userId = 'test-reseller-1';
      req.session.userEmail = email;
      req.session.userName = 'Revendedora Teste';
      req.session.userRole = 'reseller';
      req.session.resellerId = 'test-reseller-1';
      req.session.tenantId = 'test-admin-default';
