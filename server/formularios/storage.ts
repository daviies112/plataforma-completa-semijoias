...
export interface IStorage {
  ...
  getFormSubmissionById(id: string): Promise<FormSubmission | null>;
  ...
  // App Settings
  getAppSettings(tenantId: string): Promise<AppSettings | null>;
...
export class DatabaseStorage implements IStorage {
  ...
  async getAppSettings(tenantId: string): Promise<AppSettings | null> {
  ...
  async saveAppSettings(settings: InsertAppSettings): Promise<AppSettings> {
    if (!settings.tenantId) {
      throw new Error("tenantId is required to save app settings");
    }
    const existing = await this.getAppSettings(settings.tenantId);
  ...
  async setConfiguration(config: InsertConfigurationWhatsapp): Promise<ConfigurationWhatsapp> {
    if (!config.tenantId) {
      throw new Error("tenantId is required for WhatsApp configuration");
    }
    const existing = await this.getConfiguration(config.tenantId);
