...
export class LeadTrackingService {
  ...
  async criarSessaoFormulario(
    ...
    if (!tenantId) {
      throw new Error('[leadTracking] tenantId é obrigatório para criar sessão de formulário');
    }
    try {
  ...
  async finalizarFormulario(
    ...
    try {
      ...
      const statusQualificacao = passou ? 'approved' : 'rejected';
      ...
      await db.update(leads)
        .set({
          ...
          qualificationStatus: statusQualificacao,
