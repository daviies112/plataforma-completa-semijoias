...
// secure: false é OBRIGATÓRIO quando sameSite é 'lax' e não estamos forçando HTTPS
// proxy: true permite que Express confie no x-forwarded-proto do proxy HTTPS
app.use(session({
  ...
  resave: true, // Forçar salvamento para garantir persistência inicial
  saveUninitialized: false,
  proxy: true, 
  rolling: true, // Renovação de expiração a cada requisição
  cookie: {
    secure: false, // OBRIGATÓRIO com sameSite: 'lax' via HTTP/Tunnel
    httpOnly: true,
    sameSite: 'lax',
...
(async () => {
  ...
  // PROTEÇÃO DE ROTAS: A proteção é gerenciada dentro de registerRoutes (routes.ts)
  // para permitir que rotas públicas sejam registradas antes do middleware de auth.
