...
// Re-export from tenantUtils so existing imports continue to work.
// The canonical implementation lives in tenantUtils.ts (no circular deps).
export { getTenantLookupList } from './tenantUtils.js';
...
function determinePipelineStatus(journey: Partial<LeadJourney> & { formularioEnvio?: any; assinatura?: AssinaturaData }): string {
  ...
  // 1. Extreme End Cases: Sold or Signed
  if (meeting?.resultadoReuniao === 'vendido' || meeting?.resultadoReuniao === 'fechado') {
    return 'consultor';
  }
  if (assinatura) {
    const s = (assinatura.status || '').toLowerCase().trim();
    if (s === 'completed' || s === 'signed' || s === 'active' || s === 'pago') return 'revendedora';
    if (s === 'pending') return 'assinatura-pendente';
  }

  // 2. Meeting Status (Priority 1 - Tabela reunioes)
  if (meeting) {
    const s = (meeting.status || '').toLowerCase().trim();
    if (s === 'realizada' || s === 'concluida') return 'reuniao-completo';
    if (s === 'nao_compareceu' || s === 'naocompareceu' || meeting.compareceu === false) return 'reuniao-nao-compareceu';
    if (s === 'agendada' || s === 'confirmada' || s === 'agendado') return 'reuniao-agendada';
    if (s === 'pendente') return 'reuniao-agendada'; // pendente agora vai para a mesma coluna de agendada
  }

  // 3. CPF Status (Priority 2 - Tabela cpf_compliance_results)
  if (cpfData) {
    const s = (cpfData.status || '').toLowerCase().trim();
    const isApproved = s === 'approved' || s === 'aprovado' || cpfData.aprovado === true;
    const isRejected = s === 'rejected' || s === 'reprovado' || s === 'reprovare' || cpfData.aprovado === false;
    
    if (isApproved) return 'cpf-aprovado';
    if (isRejected) return 'cpf-reprovado';
  }

  // 4. Form Status (Priority 3 - Tabela form_submissions)
  if (form) {
    const p = form.passed as any;
    const isRejected = p === false || p === 'false' || p === 'rejected' || p === 'reprovado' || p === 0;
    const isApproved = p === true || p === 'true' || p === 'approved' || p === 'aprovado' || p === 1;
    
    if (isRejected) return 'formulario-reprovado';
    if (isApproved) return 'formulario-aprovado';
    return 'formulario-aprovado'; // If submitted but result unclear, mark as approved
  }

  // 5. Tracking Status (Priority 4 - Tabela form_submissions_compliance_tracking)
  if (formularioEnvio && !form) {
    return 'formulario-nao-preenchido';
  }

  // 6. Initial Contact (Default - Tabela dados_cliente)
...
function createTimeline(journey: Partial<LeadJourney> & { formularioEnvio?: any }): TimelineEvent[] {
  ...
  if (journey.form) {
    ...
    const passedValue = journey.form.passed as any;
    const isFormRejected = passedValue === false || 
                           (passedValue as any) === 'false' ||
                           (passedValue as any) === 'rejected' ||
                           (passedValue as any) === 'reprovado' ||
                           (passedValue as any) === 0;
    const isFormApproved = passedValue === true || 
                           (passedValue as any) === 'true' ||
                           (passedValue as any) === 'approved' ||
                           (passedValue as any) === 'aprovado' ||
                           (passedValue as any) === 1;
  ...
  if (journey.meeting) {
    ...
    let meetingStage = 'reuniao-agendada'; // padrão: agendada (coluna reuniao-pendente foi removida)
    ...
    if (journey.meeting.resultadoReuniao) {
      ...
      meetingTitle = 'Reunião Agendada';
      meetingStage = 'reuniao-agendada'; // pendente → agendada (coluna removida)
      meetingDescription = 'Aguardando confirmação';
    }
...
async function fetchDadosCliente(tenantId: string): Promise<Map<string, any>> {
  ...
  try {
    ...
    // Use .in() to query all tenant_id variants (handles emerick/emericks transition)
    query = query.in('tenant_id', tenantVariants);
    ...
    console.log(`📊 [LeadJourneyAggregator] Query result for ${tenantId}: ${data?.length || 0} rows`);
    ...
    for (const cliente of (data || [])) {
      ...
      console.log(`📱 [LeadJourneyAggregator] Normalizing: ${cliente.telefone} -> ${telefoneNorm}`);
      if (telefoneNorm) {
...
async function fetchN8nChatHistories(tenantId: string): Promise<Map<string, ChatHistoryData>> {
  ...
  try {
    const { data, error } = await supabase
      ...
      .select('*')
      .in('tenant_id', getTenantLookupList(tenantId)) // 🔐 Added tenant isolation
...
async function fetchFormSubmissions(tenantId: string): Promise<Map<string, any>> {
  ...
  try {
    ...
    // Filter by tenant_id - use variant list to handle slug migration (emerick/emericks)
    if (tenantId === 'system') {
  ...
  try {
    ...
    if (error) {
      if (error.code === '42P01' || error.message.includes('does not exist')) {
        ...
        altQuery = altQuery.in('tenant_id', tenantVariants);
...
async function fetchReunioes(tenantId: string): Promise<Map<string, any>> {
  ...
  for (const tableName of tableNames) {
    try {
      ...
      // Filter by tenant_id variants to handle slug migration (emerick/emericks)
      const tenantVariants = getTenantLookupList(tenantId);
      query = query.in('tenant_id', tenantVariants);
...
/**
 * Fetches data from form_submissions_compliance_tracking table
 * Used to track forms that were sent but not yet filled or processed
 */
async function fetchFormSubmissionsComplianceTracking(tenantId: string): Promise<Map<string, any>> {
  if (!tenantId || tenantId === 'default-tenant') {
    console.log('⚠️ [LeadJourneyAggregator] tenantId inválido para fetchFormSubmissionsComplianceTracking - retornando vazio');
    return new Map();
  }
  ...
  try {
    // 🔍 ENHANCEMENT: Proper tenant filtering including 'system' fallback
    let query = supabase
    ...
    if (tenantId === 'system') {
      ...
      // Other tenants only get their own data + system data (as n8n often uses 'system' tenant)
      query = query.or(`tenant_id.eq.${tenantId},tenant_id.eq.system`);
    }
    
    let { data, error } = await query;
      
    // 🚑 FALLBACK: If error or no data found for non-system tenant, try using system client
    // This is because RLS might prevent the tenant client from seeing 'system' records
    if ((error || (!data || data.length === 0)) && tenantId !== 'system') {
      console.log(`ℹ️ [LeadJourneyAggregator] Record not found with tenant client for ${tenantId}, trying system client...`);
      const systemSupabase = await getClientSupabaseClientStrict('system');
      if (systemSupabase) {
        const { data: sysData, error: sysError } = await systemSupabase
          .from('form_submissions_compliance_tracking')
          .select('*')
          .or(`tenant_id.eq.${tenantId},tenant_id.eq.system`) // Corrected to use current tenantId and system
          .order('created_at', { ascending: false })
          .limit(1000);
          
        if (!sysError && sysData && sysData.length > 0) {
          data = sysData;
          console.log(`✅ [LeadJourneyAggregator] Found ${sysData.length} records using system client`);
        }
      }
    }
    ...
    if (error && !data) {
      if (error.code === '42P01' || error.message.includes('does not exist')) {
        console.log('ℹ️ [LeadJourneyAggregator] Tabela form_submissions_compliance_tracking não existe');
        return new Map();
      }
      console.error('❌ [LeadJourneyAggregator] Erro ao buscar form_submissions_compliance_tracking:', error.message);
      return new Map();
    }
    
    const trackingMap = new Map<string, any>();
    for (const record of (data || [])) {
      // Handle the phone number which might have @s.whatsapp.net
      let phone = record.telefone || '';
      if (phone.includes('@')) {
        phone = phone.split('@')[0];
      }
      const telefoneNorm = normalizePhone(phone);
      
      if (telefoneNorm && !trackingMap.has(telefoneNorm)) {
        trackingMap.set(telefoneNorm, record);
      }
    }
    
    console.log(`✅ [LeadJourneyAggregator] Carregados ${trackingMap.size} registros de form_submissions_compliance_tracking (tenant: ${tenantId})`);
    return trackingMap;
  } catch (error: any) {
    console.error('❌ [LeadJourneyAggregator] Exceção ao buscar form_submissions_compliance_tracking:', error.message);
    return new Map();
  }
}
...
async function fetchFormularioEnvios(tenantId: string): Promise<Map<string, any>> {
  ...
  try {
    ...
    // 🔐 SECURITY FIX: ALWAYS filter by tenant_id - supports aliases
    const lookups = getTenantLookupList(tenantId);
    if (tenantId === 'system') {
      query = query.or(`tenant_id.eq.${tenantId},tenant_id.is.null`);
    } else {
      const orFilter = lookups.map(t => `tenant_id.eq.${t}`).join(',');
      query = query.or(`${orFilter},tenant_id.eq.system`);
    }
    ...
    for (const envio of (data || [])) {
      const telefoneNorm = normalizePhone(envio.telefone_normalizado || envio.telefone);
...
async function fetchAssinaturaContracts(tenantId: string): Promise<Map<string, AssinaturaData>> {
  ...
  try {
    ...
    if (error && (error.code === '42P01' || error.message.includes('does not exist') || error.message.includes('schema cache'))) {
      ...
      // 🔐 SECURITY FIX: ALWAYS filter by tenant_id - supports aliases
      fallbackQuery = fallbackQuery.in('tenant_id', getTenantLookupList(tenantId));
...
export async function aggregateLeadJourneys(tenantId: string): Promise<LeadJourney[]> {
  ...
  const [clientesMap, submissionsMap, cpfMap, reunioesMap, dashboardMap, formularioEnviosMap, trackingMap, assinaturaMap, chatHistoriesMap] = await Promise.all([
    ...
    fetchFormSubmissionsComplianceTracking(tenantId),
  ...
  for (const phone of trackingMap.keys()) allPhones.add(phone);
  ...
  for (const telefoneNorm of allPhones) {
    ...
    // Get formulario_envios and tracking data for this phone
    const staticEnvio = formularioEnviosMap.get(telefoneNorm);
    const trackingRecord = trackingMap.get(telefoneNorm);
    
    // Priority to trackingRecord as it's the more modern table (connected to compliance)
    const formularioEnvio = trackingRecord || staticEnvio;
    ...
    const nome = cliente?.NomeWpp || cliente?.nomewpp || cliente?.nome || submission?.contact_name || cpfResult?.nome || formularioEnvio?.nome || chatHistoryData?.extractedName || dashboardRecord?.nome_cliente || dashboardRecord?.nome || 'Sem nome';
    
    if (telefoneNorm === '+5531992267220' || telefone === '553192267220@s.whatsapp.net') {
      console.log('🚨 [DEBUG NAME RESOLUTION]');
      console.log(' - cliente exists?', !!cliente);
      if (cliente) {
        console.log(' - keys:', Object.keys(cliente).join(', '));
        console.log(' - NomeWpp:', cliente.NomeWpp);
        console.log(' - nomewpp:', cliente.nomewpp);
        console.log(' - nome:', cliente.nome);
      }
      console.log(' - resolved nome:', nome);
    }
    ...
    const formularioEnvioData: FormularioEnvioData | undefined = formularioEnvio ? {
      ...
      formId: formularioEnvio.form_id || formularioEnvio.formId || formularioEnvio.submission_id,
      ...
      ultimaTentativa: formularioEnvio.ultima_tentativa || formularioEnvio.last_attempt_at || formularioEnvio.updated_at,
    ...
    const orphanJourney: LeadJourney = {
      ...
      timeline: [
        ...
        ...(orphanFormData ? (() => {
          ...
          const isFormRejected = String(passedValue) === 'false' || 
                                 String(passedValue) === 'rejected' ||
                                 String(passedValue) === 'reprovado' ||
                                 String(passedValue) === '0';
          const isFormApproved = String(passedValue) === 'true' || 
                                 String(passedValue) === 'approved' ||
                                 String(passedValue) === 'aprovado' ||
                                 String(passedValue) === '1';
