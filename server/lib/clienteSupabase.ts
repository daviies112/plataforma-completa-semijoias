...
export async function saveComplianceToClienteSupabase(result: CPFComplianceResult, tenantId?: string): Promise<{ success: boolean; error?: string }> {
  try {
    ...
    // Normalizar tenantId para lowercase para consistência no banco
    const normalizedTenantId = tenantId ? tenantId.toLowerCase() : null;
    ...
    const { data, error } = await supabase
      ...
      .insert({
        ...
        tenant_id: normalizedTenantId,               // ✅ FIXADO: gravar tenant para aparecer no Kanban
