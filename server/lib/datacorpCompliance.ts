...
export async function checkCompliance(
  ...
  if (submissionId && useSupabaseMaster && !forceRefresh) {
    ...
    if (existingSubmissionCheck) {
      ...
      const checkData = existingSubmissionCheck as any;
      return {
        checkId: existingSubmissionCheck.id,
        personName: checkData.person_name || checkData.personName || existingPayload?._basic_data?.Result?.[0]?.BasicData?.Name || null,
        personCpf: checkData.person_cpf || checkData.personCpf || formatCPF(normalizedCPF),
        status: existingSubmissionCheck.status as ComplianceStatus,
        riskScore: typeof riskScoreValue === 'number' ? riskScoreValue : parseFloat(String(riskScoreValue)),
        fromCache: true,
        ...
        consultedAt: new Date(checkData.consulted_at || checkData.consultedAt),
        expiresAt: new Date(checkData.expires_at || checkData.expiresAt),
      };
    }
