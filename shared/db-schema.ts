...
export const cacheConfig = pgTable('cache_config', {
  id: serial('id').primaryKey(),
  progressiveTtlEnabled: boolean('progressive_ttl_enabled').default(true),
  accessThresholdHigh: integer('access_threshold_high').default(100),
  accessThresholdMedium: integer('access_threshold_medium').default(50),
  accessThresholdLow: integer('access_threshold_low').default(10),
  ttlHigh: integer('ttl_high').default(3600),
  ttlMedium: integer('ttl_medium').default(1800),
  ttlLow: integer('ttl_low').default(600),
  ttlDefault: integer('ttl_default').default(3600),
  batchInvalidationEnabled: boolean('batch_invalidation_enabled').default(true),
  batchInvalidationDelay: integer('batch_invalidation_delay').default(1000),
  cacheWarmingEnabled: boolean('cache_warming_enabled').default(false),
  cacheWarmingTables: text('cache_warming_tables').array(),
  compressionEnabled: boolean('compression_enabled').default(true),
  compressionThreshold: integer('compression_threshold').default(1024),
  createdAt: timestamp('created_at').defaultNow(),
  updatedAt: timestamp('updated_at').defaultNow()
});
...
export const monitoringConfig = pgTable('monitoring_config', {
  ...
  monitoringInterval: integer('monitoring_interval').default(60000),
  redisCommandsDaily: integer('redis_commands_daily').default(0),
  redisWarningThreshold: integer('redis_warning_threshold').default(400000),
  redisCriticalThreshold: integer('redis_critical_threshold').default(480000),
  supabaseBandwidthMonthly: integer('supabase_bandwidth_monthly').default(0),
  supabaseStorageLimit: integer('supabase_storage_limit').default(5242880),
  supabaseWarningThreshold: integer('supabase_warning_threshold').default(80),
  supabaseCriticalThreshold: integer('supabase_critical_threshold').default(95),
  alertsEnabled: boolean('alerts_enabled').default(true),
  alertEmail: text('alert_email'),
  autoActionsEnabled: boolean('auto_actions_enabled').default(false),
  createdAt: timestamp('created_at').defaultNow(),
  updatedAt: timestamp('updated_at').defaultNow()
});
...
export const insertCompletionPageSchema = createInsertSchema(completionPages);

export const insertAppSettingsSchema = createInsertSchema(appSettings);

export const insertConfigurationWhatsappSchema = createInsertSchema(configurationsWhatsapp);

export const insertLeadSchema = createInsertSchema(leads);

export const insertWhatsappLabelSchema = createInsertSchema(whatsappLabels);

export const insertFormularioSessaoSchema = createInsertSchema(formularioSessoes);

export const insertLeadHistoricoSchema = createInsertSchema(leadHistorico);

export const insertDadosClienteSchema = createInsertSchema(dadosCliente);

export const insertKanbanLeadSchema = createInsertSchema(kanbanLeads);
...
export type InsertCompletionPage = typeof completionPages.$inferInsert;
export type CompletionPage = typeof completionPages.$inferSelect;

export type InsertAppSettings = typeof appSettings.$inferInsert;
export type AppSettings = typeof appSettings.$inferSelect;

export type InsertConfigurationWhatsapp = typeof configurationsWhatsapp.$inferInsert;
export type ConfigurationWhatsapp = typeof configurationsWhatsapp.$inferSelect;

export type InsertLead = typeof leads.$inferInsert;
export type Lead = typeof leads.$inferSelect;

export type InsertWhatsappLabel = typeof whatsappLabels.$inferInsert;
export type WhatsappLabel = typeof whatsappLabels.$inferSelect;

export type InsertFormularioSessao = typeof formularioSessoes.$inferInsert;
export type FormularioSessao = typeof formularioSessoes.$inferSelect;

export type InsertLeadHistorico = typeof leadHistorico.$inferInsert;
export type LeadHistorico = typeof leadHistorico.$inferSelect;

export type InsertDadosCliente = typeof dadosCliente.$inferInsert;
export type DadosCliente = typeof dadosCliente.$inferSelect;

export type InsertKanbanLead = typeof kanbanLeads.$inferInsert;
...
export const insertFormularioSchema = createInsertSchema(formularios);

export type InsertFormulario = typeof formularios.$inferInsert;
...
export const insertDashboardCompletoV5BaseSchema = createInsertSchema(dashboardCompletoV5Base, {});

export const insertTenantSchema = createInsertSchema(tenantsRegistry, {}).extend({
...
export const insertComplianceUserSchema = createInsertSchema(complianceUsers, {}).extend({
...
export const insertAuditLogSchema = createInsertSchema(complianceAuditLog, {}).extend({
  action: z.string().optional(),
  ipAddress: z.string().optional(),
});

export const insertFormSubmissionComplianceTrackingSchema = createInsertSchema(formSubmissionsComplianceTracking, {});
...
export const meetingConfirmationPagesOld = pgTable("meeting_confirmation_pages_old", {
  id: uuid("id").primaryKey().default(sql`gen_random_uuid()`),
...
export const meetingTypesOld = pgTable("meeting_types_old", {
  id: uuid("id").primaryKey().default(sql`gen_random_uuid()`),
  ...
  confirmationPageId: uuid("confirmation_page_id").references(() => meetingConfirmationPagesOld.id, { onDelete: "set null" }),
  isPublic: boolean("is_public").default(false),
  isActive: boolean("is_active").default(true),
  createdAt: timestamp("created_at").defaultNow(),
  updatedAt: timestamp("updated_at"),
}, (table) => ({
  tenantIdx: index("idx_meeting_types_tenant_old").on(table.tenantId),
  slugIdx: index("idx_meeting_types_slug_old").on(table.slug),
}));

// Meeting tenant mapping for public access
export const meetingTenantMappingOld = pgTable("meeting_tenant_mapping_old", {
  meetingTypeId: uuid("meeting_type_id").primaryKey().references(() => meetingTypesOld.id, { onDelete: "cascade" }),
  tenantId: uuid("tenant_id").notNull(),
  slug: text("slug"),
  companySlug: text("company_slug"),
  isPublic: boolean("is_public").notNull().default(false),
  ...
  tenantIdx: index("idx_meeting_mapping_tenant_old").on(table.tenantId),
  slugIdx: index("idx_meeting_mapping_slug_old").on(table.slug),
  companySlugIdx: index("idx_meeting_mapping_company_slug_old").on(table.companySlug),
}));

// Meeting bookings (client appointments)
export const meetingBookingsOld = pgTable("meeting_bookings_old", {
  id: uuid("id").primaryKey().default(sql`gen_random_uuid()`),
  tenantId: uuid("tenant_id"),
  meetingTypeId: uuid("meeting_type_id").references(() => meetingTypesOld.id, { onDelete: "cascade" }).notNull(),
  ...
  tenantIdx: index("idx_meeting_bookings_tenant_old").on(table.tenantId),
  meetingTypeIdx: index("idx_meeting_bookings_type_old").on(table.meetingTypeId),
  scheduledIdx: index("idx_meeting_bookings_scheduled_old").on(table.scheduledDateTime),
  statusIdx: index("idx_meeting_bookings_status_old").on(table.status),
}));

// Meeting templates
export const meetingTemplatesOld = pgTable("meeting_templates_old", {
  id: uuid("id").primaryKey().default(sql`gen_random_uuid()`),
  tenantId: uuid("tenant_id").notNull(),
  name: text("name").notNull(),
  description: text("description"),
  ...
  tenantIdx: index("idx_meeting_templates_tenant_old").on(table.tenantId),
}));
...
export const insertMeetingTenantSchema = createInsertSchema(meetingTenants, {});

export const insertMeetingUsuarioSchema = createInsertSchema(meetingUsuarios, {});

export const insertReuniaoSchema = createInsertSchema(reunioes, {});

export const insertTranscricaoSchema = createInsertSchema(transcricoes, {});

export const insertGravacaoSchema = createInsertSchema(gravacoes, {});

export const insertMeetingConfirmationPageSchema = createInsertSchema(meetingConfirmationPages, {});

export const insertMeetingTypeSchema = createInsertSchema(meetingTypes, {});

export const insertMeetingTenantMappingSchema = createInsertSchema(meetingTenantMapping, {});

export const insertMeetingBookingSchema = createInsertSchema(meetingBookings, {});

export const insertMeetingTemplateSchema = createInsertSchema(meetingTemplates, {});
...
export type InsertDatacorpCheck = typeof datacorpChecks.$inferInsert;
...
export const insertServicePriceSchema = createInsertSchema(servicePrices, {});
