...
// 🔐 Helper para obter tenantId do usuário logado (localStorage user_data)
    ...
    queryFn: async () => {
      try {
        ...
        if (supabase) {
          ...
          console.log('[Calendario] Buscando reuniões do Supabase para', format(currentMonth, 'MMMM yyyy', { locale: ptBR }), '- tenant:', tenantId);

          // 🔐 MULTI-TENANT: filtrar por tenant_id para não vazar dados de outros tenants
          let query = supabase
            .from('reunioes')
            .select('*')
  ...
  useEffect(() => {
    ...
    const setupRealtime = async () => {
      try {
        ...
        // 🔐 MULTI-TENANT: filtrar eventos realtime apenas do tenant atual
        const realtimeTenantId = getCurrentTenantId();
        const realtimeFilter = realtimeTenantId ? `tenant_id=eq.${realtimeTenantId}` : undefined;

        channel = supabase
          .channel(`reunioes-calendario-changes-${realtimeTenantId || 'all'}`)
          .on(
            'postgres_changes',
            {
              event: '*',
